...
def create_table(s_box_n_bit, V_n, save=False):
    ...
    for alpha in V_n:
        for beta in V_n:
            all_trunc_diff = list(itertools.product(precalculated_delta_set[alpha], precalculated_delta_set[beta]))
            delta_alpha = precalculated_delta_set[alpha]
            delta_beta = precalculated_delta_set[beta]
            temp_x_list = list()
            temp_max_dp = list()
            ...
            # for index, alpha_1 in enumerate(delta_alpha):
            #     temp_x_list.append(set())
            #     for beta_1 in delta_beta:
            #         temp_x_list[index] = temp_x_list[index].union(set(D_table[(alpha_1, beta_1)]))
            # if temp_x_list and temp_x_list[0]:
            #     temp_x_list = list(reduce(and_, temp_x_list))
            # else:
            #     temp_x_list = []
            for differential in all_trunc_diff:
                temp_max_dp.append(len(D_table[differential]))
            TD_table[(alpha, beta)] = temp_x_list
            max_DP_table[(alpha, beta)] = max(temp_max_dp) if temp_max_dp else 0
            delta_alpha_table[(alpha, beta)] = delta_alpha
            delta_beta_table[(alpha, beta)] = delta_beta



            # for i in V_n:
            #     for alpha_pos in alpha_set:
            #         for beta_pos in beta_set:
            #             s_box_result = s_box_2_bit.calculate_result(i)
            #             s_box_alpha_result = s_box_2_bit.calculate_result(i ^ alpha_pos)
            #             if s_box_alpha_result == s_box_result ^ beta_pos:
            #                 temp_x_list.append(i)
    ...
    for i in result_table:
        res['(' + bin(i[0])[2:] + ', ' + bin(i[1])[2:] + ')'] = result_table[i]

    df = pd.DataFrame.from_dict(data=res, orient='index', columns=["DP(a, b)", "maxDP(a', b')", "TDP(a, b)", 'TD(a,b)', 'delta alpha', 'delta beta'])
    if save:
        if s_box_n_bit.name:
